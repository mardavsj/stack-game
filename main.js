...
let lastTapTime = 0;
const tapThreshold = 200; // Threshold for rapid taps (200ms is good)

function eventHandler() {
    const currentTime = Date.now();
    const timeDiff = currentTime - lastTapTime;

    // Allow tap if enough time has passed since the last tap
    if (timeDiff < tapThreshold) {
        return; // Block rapid taps (e.g., fast, accidental multiple clicks)
    }
...
function splitBlockAndAddNextOneIfOverlaps() {
    ...
    if (overlap > 0) {
        ...
        // Play sound after placement
        if (!isSoundPlaying) {
            placeSound.play();
            isSoundPlaying = true;  // Flag the sound as playing
        }

        // Reset the sound flag after the sound finishes (to avoid multiple plays)
        placeSound.onended = () => {
            isSoundPlaying = false;  // Reset flag when sound finishes playing
        };
    } else {
        missedTheSpot();
    }
